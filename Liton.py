...
def login():
	os.system("clear")
	cetak(nel(f'   {P2}Login Cookies First Bro\n\n              {H2}--[LITON ]--',title=f'{P2} {H2}[ {P2}🌺Assalamu Walaikum🌺 {H2}]',width=54,padding=(1,4),style='blue'))
	cetak(nel(f'{P2} Download Cookies King Liton',subtitle=f'{P2}┌─[ Cookies ]',subtitle_align='left',width=54,padding=1,style='blue'))
	cookie = input(f"{P}   └──> : {H}")
	try:
		...
		cetak(nel(f'{P2} LITON LOGIN SUCCESSFUL',width=24,style=f"#00FF00"));time.sleep(2)
		bot_share()
	except:
		os.system("rm token.txt cookie.txt")
		cetak(nel(f'{P2} TOOLS OWNER LITON AHMED',width=22,style=f"#00FF00"));time.sleep(1.5) 
		login()
...
def bot_share():
	...
	try:
		...
		os.system("rm token.txt cookie.txt")
		cetak(nel(f'{P2} TOOLS OWNER LITON!!',width=22,style=f"#00FF00"));time.sleep(1.5)
		login()
	os.system('clear')
	logo_menu()
